...
const signUpController = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: createdUser._id.toString(), email: createdUser.email }, secret, { expiresIn: '1h' });
...
const signInContoller = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: user._id.toString(), email: user.email }, secret, { expiresIn: '1h' });
        res.cookie('token', token, {
            httpOnly: true,
            sameSite: 'lax',
            secure: process.env.NODE_ENV === 'production',
